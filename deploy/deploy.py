...
Parameter:
    ...
    installAll:     check the environment, deploy all service
    startAll:       check service port, start all service 
    stopAll:        stop all service
    startServer:       start cdapp-server service
    stopServer:        stop cdapp-server service
    startWeb:       start cdapp-web and cdapp-h5 service
    stopWeb:        stop cdapp-web and cdapp-h5 service

Attention:
    1. Need to install python3.6, jdk, mysql, PyMySQL first
    2. Need to ensure a smooth network
    3. You need to install nginx,dos2unix; if it is not installed, the installation script will automatically install these components, but this may fail.
    '''
    print (helpMsg)
    return
